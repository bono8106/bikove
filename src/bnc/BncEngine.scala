...
abstract class BncEngine {
  ...
  abstract class TCustomBncPlayer(val allowLeadZero: Boolean) {
    ...
    def addSuggestion(sugg: TBncSuggestion): Unit
  ...
  type TNumberList = collection.mutable.ArrayDeque[TBncNumber]
  ...
  abstract class TComputerBncPlayer(allowLeadZero: Boolean) extends TCustomBncPlayer(allowLeadZero) {
    ...
    protected def generateList(): Unit
    protected def freeList(): Unit
    protected def shrinkList(suggestion: TBncNumber, mattch: TBncMatch): Unit
    ...
    def guessNumber: TBncNumber
    def moveMode(move: TBncMove): TBncMoveMode
    def variants: TNumber = listCount
